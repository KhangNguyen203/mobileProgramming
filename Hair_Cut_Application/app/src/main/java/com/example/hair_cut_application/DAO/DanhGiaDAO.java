...
public class DanhGiaDAO {
    ...
    public void themDanhGia(DanhGiaDTO danhGiaDTO){
        ...
        contentValues.put(CreateDatabase.TB_DANHGIA_TGIANDG,danhGiaDTO.getTgDG());
    ...
    public ArrayList<DanhGiaDTO> getDanhGiaHaveUser(String shopID){
        ...
        String query = "select LICHSU.userID, DANHGIA.shopRating, DANHGIA.shopReview, DANHGIA.thoiGianDanhGia " +
        ...
        while(!c.isAfterLast()) {
            ...
            String tgDG = c.getString(3);
            LichSuDTO lichSuDTO = new LichSuDTO(userId);
            DanhGiaDTO danhGiaDTO = new DanhGiaDTO(lichSuDTO, ratingShop, reviewShop, 0.0f, null, tgDG);
            ds.add(danhGiaDTO);
            c.moveToNext();
        }
        c.close();
    ...
    public ArrayList<DanhGiaDTO> getDanhGiaStylist(String stylistID){
        ArrayList<DanhGiaDTO> ds = new ArrayList<>();
        this.database = createDatabase.getReadableDatabasee();
        String query = "select LICHSU.userID, DANHGIA.stylistRating, DANHGIA.stylistReview, DANHGIA.thoiGianDanhGia " +
                " from DANHGIA join LICHSU on DANHGIA.lichSuID = LICHSU.id" +
        ...
        while(!c.isAfterLast()) {
            ...
            LichSuDTO lichSuDTO = new LichSuDTO(userId);
            DanhGiaDTO danhGiaDTO = new DanhGiaDTO(lichSuDTO,0.0F ,null, ratingSty, reviewSty,tgDG);
            ds.add(danhGiaDTO);
            c.moveToNext();
        }
        c.close();
